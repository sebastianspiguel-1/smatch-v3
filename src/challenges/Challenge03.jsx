  ...
  async function extractAndSaveInsights() {
    try {
      ...
      const coachLog = (profile.ai_coach_usage.interactions || [])
        .filter((i) => i.challenge === "El dev que se está apagando")
        .map(
          (i) =>
      ...
      const prompt = buildInsightExtractorPrompt(
        "El dev que se está apagando (C03)",
        conversationLog,
        coachLog,
        actionsLog
      )
      ...
      updateProfile(DEFAULT_CANDIDATE_ID, {
        ...
        challenge_history: [
          {
            challenge: "C03",
            challenge_name: "El dev que se está apagando",
      ...
      console.error("Error en insight extraction C03:", e)
    }
  }
  ...
  useEffect(() => {
    if (phase === "results" && finalScores.length > 0) {
      ...
      saveResult({
        ...
        grade: gradeData,
        timeUsed,
      }).then(() => console.log("Challenge 03 result saved"))
    }
